...
function coreFiles(name: string, type: ProjectType): ScaffoldFile[] {
  ...
  return [
    ...
    { path: 'lunira.config.json', content: luniraConfig() },
...
export async function scaffold(options: ScaffoldOptions): Promise<void> {
  ...
  await exec('git', ['commit', '-m', 'chore: init project scaffold\n\nGenerated by lunira init.'], {
    cwd: targetDir,
  });
...
export function buildGuidance(name: string, type: ProjectType): string {
  return [
    ...
    `Next steps:`,
    `  cd ${name}`,
    `  lunira setup          # configure LLM API keys`,
    `  lunira run "<intent>" # run your first pipeline`,
    `\nEdit lunira.config.json to activate skills and plugins.\n`,
  ].join('\n');
}
